...
export default function PopularProducts() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("/api/product") // API se data fetch
        const data = await res.json()
        setProducts(data.data) // API ka data state me set
      } catch (error) {
  ...
  return (
    <section className="bg-background py-12 px-4 sm:px-6 lg:px-12">
